...
public class Presenter {
    ...
    public Presenter(View view, String pathDb) { // связывает модель с конкретными представлениями
        this.view = view;
        model = new Model(pathDb);
    }

    public void LoadFromFile() { // загрузка данных
        model.load();

        if (model.currentBook().count() > 0) { // получение данных
            model.setCurrentIndex(0);
            var contact = model.currentContact();

            view.setFirstName(contact.firstName); // передача данных во view
